...
namespace OneBlog.Core.Data
{
    ...
    public class TagRepository : ITagRepository
    {
        ...
        {
            if (!Security.IsAuthorizedTo(OneBlog.Core.Rights.ViewPublicPosts))
                throw new System.UnauthorizedAccessException();

            var tags = new List<TagItem>();

            foreach (var p in OneBlog.Core.Post.ApplicablePosts)
            {
        ...
        {
            if (!Security.IsAuthorizedTo(OneBlog.Core.Rights.AccessAdminPages))
                throw new System.UnauthorizedAccessException();
            try
            {
                foreach (var p in OneBlog.Core.Post.Posts.ToArray())
                {
