...
namespace OneBlog.Core.Data
{
    ...
    public class RolesRepository : IRolesRepository
    {
        ...
        {
            if (!Security.IsAuthorizedTo(OneBlog.Core.Rights.ViewRoles))
        ...
        public IEnumerable<Group> GetRoleRights(string role)
        {
            ...
            var rightCategories = new Dictionary<OneBlog.Core.Rights, string>();
            var roleRights = OneBlog.Core.Right.GetRights(role);

            foreach (FieldInfo fi in typeof(OneBlog.Core.Rights).GetFields(BindingFlags.Static | BindingFlags.GetField | BindingFlags.Public))
            {
                OneBlog.Core.Rights right = (OneBlog.Core.Rights)fi.GetValue(null);

                if (right != OneBlog.Core.Rights.None)
                {
