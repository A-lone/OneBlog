...
namespace OneBlog.Core
{
    ...
    /// outside users. There's various areas in OneBlog where an attacker could supply an external url to the server
    ...
    /// could also use this same method to use one OneBlog instance to hammer another site by, again, passing an off-server
    ...
    /// 
    /// This class will not throw an exception if the Uri supplied points to a resource local to the running OneBlog instance.
    /// 
    ...
    internal sealed class RemoteFile
    {
        ...
        /// <param name="ignoreRemoteDownloadSettings">Set to true if RemoteFile should ignore the current OneBlog instance's remote download settings.</param>
        ...
        private void CheckCanDownload()
        {
            if (!this.IgnoreRemoteDownloadSettings && !BlogSettings.Instance.AllowServerToDownloadRemoteFiles)
            {
                ...
                {
                    throw new SecurityException("OneBlog is not configured to allow remote file downloads.");
                }
            }
        }
        ...
        /// <summary>
        /// Gets whether the Uri property is pointed at a resource local to the running OneBlog instance.
        /// </summary>
        /// <remarks>
