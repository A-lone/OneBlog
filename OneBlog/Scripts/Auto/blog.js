...
OneBlog = {
    ...
    setFlag: function (iso) {
        if (iso.length > 0)
            OneBlog.comments.flagImage.src = OneBlogRes.webRoot + "Content/images/blog/flags/" + iso + ".png";
        else
            OneBlog.comments.flagImage.src = OneBlogRes.webRoot + "Content/images/blog/pixel.gif";
    }
	,
    ...
    showCommentPreview: function () {
        ...
        this.$('commentPreview').innerHTML = '<img src="' + OneBlogRes.webRoot + 'Content/images/blog/ajax-loader.gif" width="24" height="24" alt="Loading" />';
    ...
    endShowPreview: function (arg, context) {
        if (OneBlog.$('commentPreview')) {
            OneBlog.$('commentPreview').innerHTML = arg;
        }
    }
	,
    toggleCommentSavingIndicators: function (bSaving) {
        OneBlog.$("btnSaveAjax").disabled = bSaving;
        OneBlog.$("ajaxLoader").style.display = bSaving ? "inline" : "none";
        OneBlog.$("status").className = "";
        OneBlog.$("status").innerHTML = "";
        if (!bSaving) {
            if (OneBlog.$('commentPreview')) {
                OneBlog.$('commentPreview').innerHTML = "";
            }
            OneBlog.composeComment();
        }
    }
    ,
    onCommentError: function (error, context) {
        OneBlog.toggleCommentSavingIndicators(false);
    ...
    addComment: function (preview) {
        ...
        if (!isPreview) {
            OneBlog.toggleCommentSavingIndicators(true);
            this.$("status").innerHTML = OneBlogRes.i18n.savingTheComment;
        }
        ...
        var notify = OneBlog.$("cbNotify") ? OneBlog.$("cbNotify").checked : false;
        var replyToId = OneBlog.comments.replyToId ? OneBlog.comments.replyToId.value : "";
        ...
        var avatarInput = OneBlog.$("avatarImgSrc");
        var avatar = (avatarInput && avatarInput.value) ? avatarInput.value : "";

        var callback = isPreview ? OneBlog.endShowPreview : OneBlog.appendComment;
        ...
        WebForm_DoCallback(OneBlog.comments.controlId, argument, callback, 'comment', OneBlog.onCommentError, false);
    ...
    replyToComment: function (id) {
        ...
        if (!id || id == '' || id == null || id == '00000000-0000-0000-0000-000000000000') {
            ...
            // show cancel
            OneBlog.$('cancelReply').style.display = '';

            // move to nested position
            var parentComment = OneBlog.$('id_' + id);
        ...
        OneBlog.comments.nameBox.focus();
    }
	,
    appendComment: function (args, context) {
        if (context == "comment") {
            ...
            if (args == "RecaptchaIncorrect" || args == "SimpleCaptchaIncorrect") {
                ...
                OneBlog.toggleCommentSavingIndicators(false);
                OneBlog.$("status").className = "success";

                if (!OneBlog.comments.moderation)
                    OneBlog.$("status").innerHTML = OneBlogRes.i18n.commentWasSaved;
                else
                    OneBlog.$("status").innerHTML = OneBlogRes.i18n.commentWaitingModeration;
        ...
        OneBlog.$("btnSaveAjax").disabled = false;
    }
	,
    validateAndSubmitCommentForm: function () {

        if (OneBlog.comments.nameBox.value.length < 1) {
            OneBlog.$("status").innerHTML = "Name is required.";
            OneBlog.$("status").className = "warning";
            OneBlog.$("txtName").focus();
            return false;
        }
        if (OneBlog.comments.emailBox.value.length < 1) {
            OneBlog.$("status").innerHTML = "Email is required.";
            OneBlog.$("status").className = "warning";
            OneBlog.$("txtEmail").focus();
            return false;
        }
        if (OneBlog.comments.contentBox.value.length < 1) {
            OneBlog.$("status").innerHTML = "Comment is required.";
            OneBlog.$("status").className = "warning";
            OneBlog.$("txtContent").focus();
            return false;
        }

        //var bBuiltInValidationPasses = Page_ClientValidate('AddComment');
        //var bNameIsValid = OneBlog.comments.nameBox.value.length > 0;

        //document.getElementById('spnNameRequired').style.display = bNameIsValid ? 'none' : '';
        //var bAuthorNameIsValid = true;
        //if (OneBlog.comments.checkName) {
        //    var author = OneBlog.comments.postAuthor;
        //    var visitor = OneBlog.comments.nameBox.value;
    ...
    addBbCode: function (v) {
        try {
            var contentBox = OneBlog.comments.contentBox;
    ...
    search: function (root, searchfield) {
        ...
        var searchPageExtension = typeof OneBlogRes.fileExtension === "undefined" ? ".aspx" : OneBlogRes.fileExtension;
    ...
    rate: function (blogId, id, rating) {
        this.createCallback("rating.axd?id=" + id + "&rating=" + rating, OneBlog.ratingCallback, blogId);
    }
	,
    ratingCallback: function (response) {
        ...
        if (status == "OK") {
            ...
            alert(OneBlogRes.i18n.ratingHasBeenRegistered);
        }
        else if (status == "HASRATED") {
            alert(OneBlogRes.i18n.hasRated);
        }
        else {
    ...
    createCallback: function (url, callback, blogId) {
        var http = OneBlog.getHttpObject();
    ...
    updateCalendar: function (args, context) {
        var cal = OneBlog.$('calendarContainer');
        cal.innerHTML = args;
        OneBlog.Calendar.months[context] = args;
    }
	,
    toggleMonth: function (year) {
        var monthList = OneBlog.$("monthList");
    ...
    hightLightXfn: function () {
        ...
        for (i = 0; i < links.length; i++) {
            ...
            if (rel && rel != "nofollow") {
                for (j = 0; j < OneBlog.xfnRelationships.length; j++) {
                    if (rel.indexOf(OneBlog.xfnRelationships[j]) > -1) {
    ...
    showRating: function (container, id, raters, rating, blogId) {
        ...
        div.appendChild(p);
        if (raters == 0) {
            p.innerHTML = OneBlogRes.i18n.beTheFirstToRate;
        }
        else {
            p.innerHTML = OneBlogRes.i18n.currentlyRated.replace('{0}', new Number(rating).toFixed(1)).replace('{1}', raters);
        }
        ...
        for (var i = 1; i <= 5; i++) {
            ...
            a.title = OneBlogRes.i18n.rateThisXStars.replace('{0}', i.toString()).replace('{1}', i == 1 ? '' : 's');
            a.rel = "nofollow";
            a.onclick = function () {
                OneBlog.rate(blogId, id, this.innerHTML);
                return false;
            };
    ...
    applyRatings: function () {
        ...
        for (var i = 0; i < divs.length; i++) {
            if (divs[i].className == 'ratingcontainer') {
                ...
                OneBlog.showRating(divs[i], args[0], args[1], args[2], args[3]);
            }
        }
    },
    ...
    filterByAPML: function () {
        ...
        var p = document.createElement('p');
        p.innerHTML = OneBlogRes.i18n.apmlDescription;
        p.style.margin = '0px';
        div.appendChild(p);
        ...
        form.action = OneBlogRes.webRoot;
        div.appendChild(form);

        var textbox = document.createElement('input');
        textbox.type = 'text';
        textbox.value = OneBlog.getCookieValue('url') || 'http://';
        textbox.style.width = '320px';
        textbox.id = 'txtapml';
        textbox.name = 'apml';
        textbox.style.background = 'url(' + OneBlogRes.webRoot + 'Content/images/blog/apml.png) no-repeat 2px center';
        ...
        button.type = 'submit';
        button.value = OneBlogRes.i18n.filter;
        button.onclick = function () { location.href = OneBlogRes.webRoot + '?apml=' + encodeURIComponent(OneBlog.$('txtapml').value); };
        ...
        var a = document.createElement('a');
        a.innerHTML = OneBlogRes.i18n.cancel;
        a.href = 'javascript:void(0)';
        a.onclick = function () { document.body.removeChild(OneBlog.$('layer')); document.body.removeChild(OneBlog.$('apmlfilter')); document.body.style.position = ''; };
        div.appendChild(a);
    }
	,
...
OneBlog.addLoadEvent(OneBlog.hightLightXfn);
...
OneBlog.addLoadEvent(OneBlog.applyRatings);
