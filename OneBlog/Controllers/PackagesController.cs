...
public class PackagesController : ApiController
{
    ...
    public bool Refresh()
    {
        try
        {
            OneBlog.Core.Blog.CurrentInstance.Cache.Remove(Constants.CacheKey);
            return true;
        }
        catch (Exception ex)
        {
    ...
    public HttpResponseMessage SetTheme(string id)
    {
        if (!string.IsNullOrEmpty(id))
        {
            var bs = OneBlog.Core.BlogSettings.Instance;
            bs.Theme = id;
            bs.Save();
            var item = System.Web.Mvc.ViewEngines.Engines.Where(m => m.GetType() == typeof(CustomViewEngine)).FirstOrDefault();
            if (item != null)
            {
                ((CustomViewEngine)item)?.Update(id);
            }
        }
