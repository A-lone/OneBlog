angular.module('blogAdmin').controller('CategoriesController', ["$rootScope", "$scope", "$location", "$http", "$filter", "dataService", function ($rootScope, $scope, $location, $http, $filter, dataService) {
    ...
    if ($scope.id) {
        dataService.getItems('/admin/categories', { Id: $scope.id })
    ...
    $scope.load = function () {
        ...
        var p = { skip: 0, take: 0 };
        dataService.getItems('/admin/categories', p)
    ...
    $scope.save = function () {
        ...
        if ($scope.category.Id) {
            dataService.updateItem("/admin/categories/update/" + $scope.category.Id, $scope.category)
           .success(function (data) {
    ...
    $scope.processChecked = function (action, itemsChecked) {
        if (itemsChecked) {
            processChecked("/admin/categories/processchecked/", action, $scope, dataService);
        }
    }
